...
This approach uses a `view` function implementation of `supportsInterface`
The execution cost is 586 gas for any input. But contract initialization requires storing each interface (`SSTORE` is 20,000 gas)
The `ERC165MappingImplementation` contract is generic and reusable.
